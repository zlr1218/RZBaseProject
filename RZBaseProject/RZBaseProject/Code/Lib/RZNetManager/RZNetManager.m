...
+ (RZURLSessionTask *)rz_uploadImgWithURL:(NSString *)urlStr parameters:(NSDictionary *)parameterDic ImgArray:(NSArray *)imgArray success:(RZRequestSuccessBlock)success failed:(RZRequestFailBlock)failed progress:(RZDownloadProgress)progress {
    ...
    sessionTask = [[self sharedAFManager] POST:urlStr parameters:parameterDic constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        ...
        for (int i = 0; i < imgArray.count; i++)
        {
            ...
            if (imgData != nil)
            {   // 图片数据不为空才传递
                [formData appendPartWithFileData:imgData name:[NSString stringWithFormat:@"picflie%ld",(long)i] fileName:imgFileName mimeType:@" image/jpeg"];
            }
        }
